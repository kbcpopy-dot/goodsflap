...
app.use((req,res,next)=>{res.set('X-Content-Type-Options','nosniff');res.set('Referrer-Policy','same-origin');res.set('Cache-Control','no-store');const requestOrigin=`${req.headers['x-forwarded-proto']||req.protocol}://${req.headers.host}`;const allowedOrigin=process.env.PUBLIC_URL||requestOrigin;if(req.method!=='GET'&&req.headers.origin&&req.headers.origin!==allowedOrigin)return res.status(403).json({error:'허용되지 않은 요청 출처입니다.'});let sid=req.headers.cookie?.match(/(?:^|; )artell_session=([a-f0-9]{64})(?:;|$)/)?.[1];if(!sid){sid=randomBytes(32).toString('hex');res.cookie('artell_session',sid,{httpOnly:true,sameSite:'lax',secure:allowedOrigin.startsWith('https:'),maxAge:30*86400000});}req.sid=sessionKey(sid);next();});
